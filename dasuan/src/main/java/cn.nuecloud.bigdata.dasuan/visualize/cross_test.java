...
import cn.nuecloud.bigdata.dasuan.analysis.cross_correlation.cross_correlation;
 ...
 */
public class cross_test implements java.io.Serializable{
        ...
        try {
            ...
            JavaRDD<String> data = myRDD.flatMap(new FlatMapFunction<Tuple2<ImmutableBytesWritable, Result>, String>() {
                ...
                public Iterable<String> call(Tuple2<ImmutableBytesWritable, Result> t) throws Exception {
                    ...
                    String Movement_Rate = Bytes.toString(t._2.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("Movement_Rate")));
                    String Move_Speed = Bytes.toString(t._2.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("Move_Speed")));
                    String Rotate_Speed = Bytes.toString(t._2.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("Rotate_Speed")));
                    String Pure_Movement = Bytes.toString(t._2.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("Pure_Movement")));

                    return Arrays.asList(Movement_Rate + "," + Move_Speed+","+Rotate_Speed+","+Pure_Movement);
                }
            });
            JavaRDD<Row> rows = data.map(new Function<String, Row>() {
                ...
                public Row call(String line) {
                    ...
                    return RowFactory.create(splited[0], splited[1],splited[2],splited[3]);
                }
            });
            List<StructField> structFields = new ArrayList<StructField>();
            structFields.add(DataTypes.createStructField("Movement_Rate", DataTypes.StringType, true));
            ...
            structFields.add(DataTypes.createStructField("Pure_Movement", DataTypes.StringType, true));
            StructType structType = DataTypes.createStructType(structFields);
            personDF = sqlContext.createDataFrame(rows, structType);
            //        personDF.show();
            DataFrame reJavaRDD = cross_correlation.cross(sc, personDF);
            reJavaRDD.show();
...
//            st.executeUpdate(sql);
    ...
    public static void main(String args[]) {
        cross_test ttHbase = new cross_test();
        ttHbase.testFrame();
    }
}