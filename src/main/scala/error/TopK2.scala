...
/** * 实现并行堆排序算法 * @author guyadong * @param <A> * @param <B> * @param <S> */
class HeapSort[A, B, S <: Iterable[A]](f: A => B)(implicit ord: Ordering[B]) {
  /** * 对l排序返回排序后的Seq * @param l 待排序集合的迭代器 * @param desc 降/升序(默认为true,降序) * @return */
  def sort(l: S, desc: Boolean = true) = HeapSort.sort(f)(l, 0, desc)

  /** * 对l排序并返回前top个结果 * @param l 待排序集合的迭代器
    * * @param top 返回最多结果数目
    * * @param desc 降/升序(默认为true,降序)
    * * @return */
  def top(l: S, top: Int, desc: Boolean = true) = HeapSort.sort(f)(l, top, desc)

  /** * 对可变集合排序，返回排序后的Seq * @param l 待排序可变集合的迭代器
    * * @param desc 降/升序(默认为true,降序)
    * * @return */
  def sort_m[M <: mutable.Seq[A]](l: M, desc: Boolean = true) = HeapSort.sort_mutable(f)(l, 0, desc)

  /** * 对可变集合l排序并返回前top个结果 * @param l 待排序可变集合的迭代器
    * * @param top 返回最多结果数目 * @param desc 降/升序(默认为true,降序) * @return */
  def top_m[M <: mutable.Seq[A]](l: M, top: Int, desc: Boolean = true) =
    HeapSort.sort_mutable(f)(l, top, desc)

  /** * 对可变集合l并行排序并返回前top个结果 * @param l 待排序可变集合的迭代器
    * * @param top 返回最多结果数目 * @param desc 降/升序(默认为true,降序) * @return */
  def top_m_par[M <: mutable.Seq[A]](l: M, top: Int, desc: Boolean = true) =
    HeapSort.top_mutable_par(f)(l, top, desc)

  /** * 对可变集合l的指定范围排序并返回排序后的Seq * @param seq 待排序可变集合
    * * @param top 返回最多结果数目 * @param desc 降/升序(默认为true,降序)
    * * @param from 待排序的起始位置 * @param until 待排序的结束位置 * @return */
  def sort_range[M <: mutable.Seq[A]](seq: M, top: Int, desc: Boolean = true)(from: Int = 0, until: Int =
  seq.length) = HeapSort.sort_mutableRange(f)(seq, top, desc)(from, until)

  /** * 对seq中两个已经排序的区段进行合并排序，将src合并到dst * @param seq 可变集合
    * * @param src 待合并的源区段(起始位置，结束位置) * @param dst 待合并的目标区段(起始位置，结束位置) * @param desc 降/升序(默认为true,降序) * @return */
  def merge2Seq(seq: mutable.Seq[A], src: (Int, Int), dst: (Int, Int), desc: Boolean = true) =
    HeapSort.merge2Seq(f)(seq, src, dst, desc)

  /** * 对seq中两个已经排序的区段进行合并排序，将src合并到dst * @param seq 可变集合
    * * @param src 待合并的源区段(起始位置，结束位置) * @param dst 待合并的目标区段(起始位置，结束位置)
    * * @param desc 降/升序(默认为true,降序) * @return */
  def merge2Seq2(seq: mutable.Seq[A], src: (Int, Int), dst: (Int, Int), desc: Boolean = true) =
    HeapSort.merge2Seq2(f)(seq, src, dst, desc)

  /** * 对seq中两个已经排序的区段进行合并排序，将src合并到dst<br> * 该算法在排序过程不申请新内存
    * * @param seq 可变集合 * @param src 待合并的源区段(起始位置，结束位置)
    * * @param dst 待合并的目标区段(起始位置，结束位置) * @param desc 降/升序(默认为true,降序) * @return */
  def merge2SeqNM(seq: mutable.Seq[A], src: (Int, Int), dst: (Int, Int), desc: Boolean = true) =
    HeapSort.merge2SeqNM(f)(seq, src, dst, desc)
}

object HeapSort {
  def sort[A, B, S <: Iterable[A]](f: A => B)(iterator: S, top: Int = 0, desc: Boolean = true)
                                  (implicit ord: Ordering[B]) = {
    val bf = iterator.toBuffer
    sort_mutable(f)(bf, top, desc)
  }

  def sort_mutable[A, B, S <: mutable.Seq[A]](f: A => B)(seq: S, top: Int = 0, desc: Boolean = true)
                                             (implicit ord: Ordering[B]) = {
    sort_mutableRange(f)(seq, top, desc)()(
      if (top < seq.length && top > 0) seq.takeRight(top) else seq
    ).reverse
  }
  ...
  def buildHeapRange[A, B](f: A => B)(seq: mutable.Seq[A], desc: Boolean)(from: Int, until: Int)
                          (implicit ord: Ordering[B]) = {
  ...
  def cmp1_gt[A, B](f: A => B)(l: A, r: A)(implicit ord: Ordering[B]) =
    ord.gt(f(l), f(r))

  def cmp1_lt[A, B](f: A => B)(l: A, r: A)(implicit ord: Ordering[B]) = ord.lt(f(l), f(r))

  def cmp_gt[A, B](f: A => B, seq: mutable.Seq[A])(l: Int, r: Int)
                  (implicit ord: Ordering[B]) = cmp1_gt(f)(seq(l), seq(r))

  def cmp_lt[A, B](f: A => B, seq: mutable.Seq[A])(l: Int, r: Int)
                  (implicit ord: Ordering[B]) = cmp1_lt(f)(seq(l), seq(r))

  private def heapify[A, B](f: A => B)(seq: mutable.Seq[A],startpos: Int, max: Int, desc: Boolean,off: Int)
                           (implicit ord: Ordering[B]): Unit = {
    def gt = (l: Int, r: Int) => cmp_gt(f, seq)(l, r)
    def lt = (l: Int, r: Int) => cmp_lt(f, seq)(l, r)
    val cmp =
      if (desc) gt else lt var largest = 0 var idx = startpos var right = 0
    var left = 0 do {
      right = off + ((idx - off + 1) << 1)
      left = right - 1
      largest =
        if (left < max && cmp(left, idx)) left
        else idx
      if (right < max && cmp(right, largest))
        largest = right if (largest != idx) {
  ...
  def swap[A](seq: mutable.Seq[A], i: Int, j: Int) = {
    val temp = seq(i)
    seq(i) = seq(j)
    seq(j) = temp
  }
  ...
  def swap3[A](seq: mutable.Seq[A], i: Int, j: Int, k: Int) = {
    val temp = seq(i)
    seq(i) = seq(j)
    seq(j) = seq(k)
    seq(k) = temp
  ...
  def _duplicateSeq[A](src: collection.Seq[A], srcPos: Int, dest: mutable.Seq[A], destPos: Int, length: Int): mutable.Seq[A] = {
    var i = 0
    while (i < length) {
  ...
  def merge2Seq2[A, B](f: A => B)(seq: mutable.Seq[A], src: (Int, Int), dst: (Int, Int), desc: Boolean)
                      (implicit ord: Ordering[B]): (Int, Int) = {
    if (!(if (desc) cmp_gt(f, seq)(dst._1, src._2 - 1)
    else cmp_lt(f, seq)(dst._1, src._2 - 1))) {
      if (if (desc) cmp_gt(f, seq)(src._1, dst._2 - 1)
      else cmp_lt(f, seq)(src._1, dst._2 - 1)) {
        val (srclen, dstlen) = ((src._2 - src._1), (dst._2 - dst._1))
        val cplen = math.min(srclen, dstlen)
        _duplicateSeq(seq, dst._1 + cplen, seq, dst._1, dstlen - cplen)
        _duplicateSeq(seq, src._2 - cplen, seq, dst._2 - cplen, cplen)
    ...
    if (!(if (desc) cmp_gt(f, seq)(dst._1, src._2 - 1)
      ...
      if (if (desc) cmp_gt(f, seq)(src._1, dst._2 - 1)
        ...
        def moveStep(from: Int, to: Int, step: Int) =
          for (i <- (if (step > 0) (from to to).reverse
          else (from to to))) seq(i + step) = seq(i)
        def moveStep(from: Int, to: Int, step: Int) = {
        ...
        swapst = swapStHead
        swapqh = swapQhDisable while (idx >= dst._1 && st >= src._1) {
          if (cmpdst(st)) {
            swapst()
            st -= 1
          } else swapqh()
  ...
  def main(args: Array[String]) {
    ...
    val l = new Array[Int](40) for (i <- 0 until 5) {
      l(i) = rnd.nextInt(100)
    } for (i <- 5 until l.length) {
      l(i) = rnd.nextInt(100)
    } for (i <- 0 to 0) {
      println("==============time ", i, "=================") val s = l.toBuffer[Int]
      println(s) val t1 = System.currentTimeMillis
      ...
      println(r1) for (i <- (r2._1 until r2._2)) {
        print(s(i) + ",")
      }
      println(r2)
