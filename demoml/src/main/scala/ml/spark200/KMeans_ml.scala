...
class K_Means(datadir: String) extends Serializable {
  ...
  def clusterCenters(k:Int,numIterations:Int) : KMeansModel= {
    ...
    val kmeans = new KMeans().setK(k).setMaxIter(numIterations).setSeed(1L)
    //训练模型
    ...
    val part = df.selectExpr("label","prediction","features").show()
    df.filter("prediction=0").selectExpr("features").show()
